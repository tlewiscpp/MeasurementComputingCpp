...
uint8_t USB_1208FS::digitalReadPort(DigitalPortID portID) {
    ...
    if (this->m_digitalPortMap.find(portID)->second != USB_1208FS::PortDirection::DigitalInput) {
        this->setDigitalPortDirection(portID, USB_1208FS::PortDirection::DigitalInput);
    }
    uint8_t allValues{0};
    if (this->m_usbDeviceHandle) {
        auto result = usbDIn_USB1208FS(this->m_usbDeviceHandle, digitalPortIDToUInt8(portID), &allValues);
        if (result < 0) {
            throw std::runtime_error("USB_1208FS::digitalReadPort: Failed to read port (error code " + toStdString(result) + ")");
        }
        return allValues;
    } else {
        return 0;
...
USB_1208FS &USB_1208FS::digitalWritePort(DigitalPortID portID, uint8_t state) {
    std::lock_guard<std::recursive_mutex> ioLock{this->m_ioMutex};
...
uint8_t USB_1208FS::digitalPortIDToUInt8(DigitalPortID portID) {
    if (portID == USB_1208FS::DigitalPortID::PortA) {
        ...
        throw std::runtime_error("USB_1208FS::digitalPortIDToUInt8: Invalid DigitalPortID");
    }
}
...
uint8_t USB_1208FS::digitalPortDirectionToUInt8(PortDirection direction) {
    if (direction == USB_1208FS::PortDirection::DigitalInput) {
        ...
        throw std::runtime_error("USB_1208FS::digitalPortDirectionToUInt8: Invalid PortDirection");
    }
}
...
bool USB_1208FS::digitalRead(DigitalPortID portID, uint8_t pinNumber) {
    ...
    if (pinNumber >= upperPinNumber) {
        throw std::runtime_error("USB_1208FS::digitalWrite: pinNumber for ports A and B must be between 0 and " + toStdString(upperPinNumber) + "(" + toStdString(static_cast<int>(pinNumber)) + " > " + toStdString(upperPinNumber));
    }
    if (this->m_digitalPortMap.find(portID)->second != USB_1208FS::PortDirection::DigitalInput) {
        this->setDigitalPortDirection(portID, USB_1208FS::PortDirection::DigitalInput);
    }
...
short USB_1208FS::analogRead(uint8_t pinNumber, USB_1208FS::VoltageRange voltageRange) {
    ...
    */
    if ( (this->m_analogInputMode == USB_1208FS::AnalogInputMode::Differential) && (pinNumber >= (BITS_PER_PORT_1208FS/2) ) ) {
        throw std::runtime_error("USB_1208FS::analogRead: analogRead pin number cannot be greater than \""
...
USB_1208FS &USB_1208FS::analogWrite(uint8_t pinNumber, uint16_t state) {
    ...
    if (pinNumber > (NUMBER_OF_ANALOG_OUTPUT_PINS - 1)) {
        throw std::runtime_error("USB_1208FS::analogWrite: analogWrite pin number exceeds maximum pin number (" + toStdString(static_cast<int>(pinNumber)) + " > " + toStdString(NUMBER_OF_ANALOG_OUTPUT_PINS - 1));
    }
...
USB_1208FS &USB_1208FS::resetDevice() {
    std::lock_guard<std::recursive_mutex> ioLock{this->m_ioMutex};
    if (this->m_usbDeviceHandle) {
        auto result = usbReset_USB1208FS(this->m_usbDeviceHandle);
        if (result < 0) {
            throw std::runtime_error("USB_1208FS::resetDevice: Failed to reset device (error code " + toStdString(result) + ")");
        }
    }
    return *this;
}
