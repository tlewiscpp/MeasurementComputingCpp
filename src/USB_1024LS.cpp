...
uint8_t USB_1024LS::digitalPortIDToUInt8(DigitalPortID portID) {
    if (portID == USB_1024LS::DigitalPortID::PortA) {
        ...
        throw std::runtime_error("USB_1024LS::digitalPortIDToUInt8: Invalid DigitalPortID");
    }
}
...
uint8_t USB_1024LS::digitalPortDirectionToUInt8(PortDirection direction) {
    if (direction == USB_1024LS::PortDirection::DigitalInput) {
        ...
        throw std::runtime_error("USB_1024LS::digitalPortDirectionToUInt8: Invalid PortDirection");
    }
}
...
USB_1024LS &USB_1024LS::digitalWritePort(DigitalPortID portID, uint8_t state) {
    std::lock_guard<std::recursive_mutex> ioLock{this->m_ioMutex};
    if (this->m_digitalPortMap.find(portID)->second != USB_1024LS::PortDirection::DigitalOutput) {
        this->setDigitalPortDirection(portID, USB_1024LS::PortDirection::DigitalOutput);
    }
    if (this->m_hidDevice) {
        uint8_t *targetPortCurrentState{&(this->m_digitalOutputTracker.find(portID)->second)};
        *targetPortCurrentState = state;
        usbDOut_USB1024LS(this->m_hidDevice, digitalPortIDToUInt8(portID), *targetPortCurrentState);
    }
    return *this;
}

uint8_t USB_1024LS::digitalReadPort(DigitalPortID portID) {
    std::lock_guard<std::recursive_mutex> ioLock{this->m_ioMutex};
    if (this->m_digitalPortMap.find(portID)->second != USB_1024LS::PortDirection::DigitalInput) {
        this->setDigitalPortDirection(portID, USB_1024LS::PortDirection::DigitalInput);
    }
    uint8_t allValues{0};
    if (this->m_hidDevice) {
        usbDIn_USB1024LS(this->m_hidDevice, digitalPortIDToUInt8(portID), &allValues);
        return allValues;
    } else {
        return 0;
    }
...
bool USB_1024LS::digitalWrite(DigitalPortID portID, uint8_t pinNumber, bool state) {
    ...
    if ( (portID == DigitalPortID::PortA) || (portID == DigitalPortID::PortB) ) {
        ...
        if (pinNumber >= upperPinNumber) {
            throw std::runtime_error("USB_1024LS::digitalWrite: pinNumber for ports A and B must be between 0 and " + toStdString(upperPinNumber) + "(" + toStdString(static_cast<int>(pinNumber)) + " > " + toStdString(upperPinNumber));
        }
    } else {
...
bool USB_1024LS::digitalRead(uint8_t pinNumber) {
    ...
    if (!result) {
        throw std::runtime_error("USB_1024LS::digitalRead: pinNumber must be between 0 and " + toStdString(PORT_C_HIGH_MAX_PIN_NUMBER) + "(" + toStdString(static_cast<int>(pinNumber)) + " > " + toStdString(PORT_C_HIGH_MAX_PIN_NUMBER));
    }
...
bool USB_1024LS::digitalWrite(uint8_t pinNumber, bool state) {
    ...
    auto result = getDigitalPortIDAndPinNumber(pinNumber, &portID, &adjustedPinNumber);
    if (!result) {
        throw std::runtime_error("USB_1024LS::digitalWrite: pinNumber must be between 0 and " + toStdString(PORT_C_HIGH_MAX_PIN_NUMBER) + "(" + toStdString(static_cast<int>(pinNumber)) + " > " + toStdString(PORT_C_HIGH_MAX_PIN_NUMBER));
    }
...
USB_1024LS & USB_1024LS::resetDevice() {
    std::lock_guard<std::recursive_mutex> ioLock{this->m_ioMutex};
    if (this->m_hidDevice) {
        auto result = usbReset_USB1024LS(this->m_hidDevice);
        if (result < 0) {
            throw std::runtime_error("USB_1208FS::resetDevice: Failed to reset counter (error code " + toStdString(result) + ")");
        }
    }
    return *this;
}
